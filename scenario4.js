...
const fruitStands = [
  ...
  { cherriesPrice: 18, peachesPrice: 85 },
...
// The fruit stand where Pelle and Kajsa bought their fruit now has no pear baskets => it's a no-go for their friend.
fruitStands.splice(cheapest.index, 1);

// To find out where Pelle and Kajsa's friend should go, we just go through the whole thing again (sans, of course, the stand that Pelle and Kajsa took all the pears from):
cheapest.price = Number.MAX_SAFE_INTEGER;
...
// The number of stands they considered is the total number of stands (sans Pelle and Kajsa's one) minus the stands without any pears.
...
console.log(`After looking at ${standCount} stands, Pelle and Kajsa's friend bought ${cheapest.fruits} at stand number ${cheapest.index + 1} for the price of ${cheapest.price} SEK.`);